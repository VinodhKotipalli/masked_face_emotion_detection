...
class KMeansClassifier:
    ...
    def processWithKmeans(self,inputs):
        ...
        if self.clusteringOutput == 'add':
            output['km_clust'] = predictedLabels
    ...
    def trainClusters(self):
        ...
        if self.clusteringOutput == 'add':
            self.__kmeansProcessedTrainingInputs['km_clust'] = clusterLabels
        elif self.clusteringOutput == 'replace':
...
if __name__ == '__main__':
    ...
    targets_test = dict()
    inputs_test = dict()
    extractedInputs_train = dict()
    extractedInputs_test = dict()
    ...
    extractor['WithoutMasks'] = NMF(n_components = 49,max_iter = 1500,init='random', random_state=0)
    extractedInputs_train['WithoutMasks'] = extractor['WithoutMasks'].fit_transform(inputs_train['WithoutMasks'])
    extractedInputs_test['WithoutMasks'] = extractor['WithoutMasks'].transform(inputs_test['WithoutMasks'])
    ...
    for k in kRange:
        ...
        for tag in tags:
            for clusteringOutput in clusteringOutputModes:
                jobs[tag][clusteringOutput][k] = multiprocessing.Process(target=getAccuracy, args=(extractedInputs_train[tag],targets_train[tag],k,clusteringOutput,extractedInputs_test[tag],targets_test[tag],tag,returnDict,classifierName))
